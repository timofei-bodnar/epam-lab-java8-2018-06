...
public class Exercise1 {
    ...
    private static ToIntBiFunction<Person, Person> createExtractorAgeOfPersonWithTheLongestFullName(
        Function<Person, String> function) {
        return (p1, p2) -> {
            if (function.apply(p1).length() > function.apply(p2).length()) {
                return p1.getAge();
            } else if (function.apply(p1).length() < function.apply(p2).length()) {
                return p2.getAge();
            } else {
                return -1;
            }
    ...
    public void getAgeOfPersonWithTheLongestFullName() {
        ...
        BiFunction<Person, Person, Integer> extractorAgeOfPersonWithTheLongestFullName =
            ((p1, p2) -> createExtractorAgeOfPersonWithTheLongestFullName(getFullName)
                .applyAsInt(p1, p2));
